function [net, info] = cnn_train_fbpconvnetep(net, imdb, getBatch, beta, delta, mu_para, block, Outer, numepoch, katype,varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% load kappa, denom, sinograms, and weight matrces for training and testing
% images here
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for epoch=start+1:opts.numEpochs
    ...
    for numepo = 1:numepoch
        ...
        if numGpus <= 1
            ...
            spmd(numGpus)
                ...
                [~, stats_val_,prof_,FBPVal] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net_) ;
            end
            net = net_{1} ;
    ...
    for indices = 1:size(imdb.images.noisy,4)
        res=vl_simplenn_fbpconvnet_eval(net,gpuArray( single(imdb.images.noisy(:,:,1,indices)) ));
        rec=gpuArray(gather(res(end-1).x)+squeeze(imdb.images.noisy(:,:,1,indices))); 
        xrlam = cat(3,xrlam,rec);
    end
    %%%%%%%%%%%%%%%%%%%%%% EP Module %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    xrla = zeros(size(ref(:),1),size(xrlam,3));
        parfor jjj = 1:size(xrlam,3)
            temp = gather(xrlam(:,:,jjj));
            R{jjj} = Reg1(sqrt(kappa(:,:,jjj)), 'beta', 2^beta, 'pot_arg', pot_arg, 'nthread', jf('ncore'));
             [xrla(:,jjj), ~] = pwls_ep_os_rlalm_l2normReg(temp(:), A, sino(:,:,jjj), R{jjj}, mu_para, jjj, 'wi', wi(:,:,jjj), ...
                         'pixmax', inf, 'isave', 'last',  'niter', nOuterIter, 'nblock', nblock, ...
                         'chat', 0, 'denom',denom(:,:,jjj), 'xtrue', imdb.images.orig(:,:,jjj), 'mask', ig.mask);
%            [xrla(:,jjj), ~] = pwls_ep_os_rlalm(temp(:), A, sino(:,:,jjj), R{jjj}, jjj, 'wi', wi(:,:,jjj), ...
%                        'pixmax', inf, 'isave', 'last',  'niter', nOuterIter, 'nblock', nblock, ...
%                        'chat', 0, 'denom',denom(:,:,jjj), 'xtrue', imdb.images.orig(:,:,jjj), 'mask', ig.mask);            
            xrlam(:,:,jjj) = reshape(xrla(:,jjj),512,512);
        end
        ttt = xrlam(:,:,aaa);
        AAA = norm(ttt(:) - ref(:)) / SqrtPix;
        fprintf('RMSE = %g \n ', AAA);
