function [net, info] = cnn_train_fbpconvnet_pure(net, imdb, getBatch, numepoch,varargin)
...
for epoch=start+1:opts.numEpochs
    ...
    for numepo = 1:numepoch
        ...
        if numGpus <= 1
            ...
            spmd(numGpus)
                ...
                [~, stats_val_,FBPVal] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net_) ;
            end
            net = net_{1} ;
    ...
    RMSE(1,epoch) = norm(col(xrlam(:,:,aaa)) - ref(:)) / SqrtPix;
    if epoch == 1
...
figure(12);
subplot(1,3,1); imshow(labelsTemp,[800 1200]); colormap(gray); axis off image;title('original');
subplot(1,3,2); imshow(imTemp,[800 1200]); colormap(gray); axis off image; title('FBP-Sparse view');
subplot(1,3,3); imshow(recTemp,[800 1200]); colormap(gray); axis off image;title('FBPConvNet');
drawnow;
pause(.1);
