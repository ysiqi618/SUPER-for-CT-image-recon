function [net, info] = cnn_train_fbpconvnetultra(net, imdb, getBatch, beta, gamma, mu_para, block, InIter, Outer, numepoch, clu, katype, ...
    decay_beta,decay_gamma,decay_epoch_beta,decay_epoch_gamma,varargin)
...
nblock = block;            % Subset Number
nIter = InIter;             % I--Inner Iteration (rlalam)
nOuterIter = Outer;     % T--Outer Iteration
CluInt = clu;            % Clustering Interval
KapType = katype; % 0: no kappa; 1: with kappa


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% load kappa, denom, sinograms, and weight matrices for 
% low dose training/validation images here
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
kappa = cat(3,kappa_low_train,kappa_low_val);
denom = cat(3,denom_low_train,denom_low_val);
sino = cat(3,sino_low_train,sino_low_val);
wi = cat(3,wi_low_train,wi_low_val);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% load pre-learnded unions of sparsifying transforms here
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mOmega = info.mOmega;
...
for epoch=start+1:opts.numEpochs
    ...
    for numepo = 1:numepoch
        fprintf('Epoch = %d of %d\n', numepo, numepoch)
        if numGpus <= 1
            [net,stats.train,prof,FBPTrain] = process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
            [~,stats.val,prof,FBPVal] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net);
            xrlam = cat(3,FBPTrain,FBPVal(:,:,max(find(imdb.images.set == 1))+1:end));
            if opts.profile
                profile('viewer') ;
                keyboard ;
            end
            ...
            spmd(numGpus)
                [net_, stats_train_,prof_,FBPTrain] = process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
                [~, stats_val_,prof_,FBPVal(:,:,101:end)] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net_) ;
            end
            net = net_{1} ;
            stats.train = sum([stats_train_{:}],2) ;
            stats.val = sum([stats_val_{:}],2) ;
            xrlam = cat(3,FBPTrain,FBPVal);
            if opts.profile
    ...
    aaa = 50;
    ref = reshape(imdb.images.orig(:,:,1,aaa),512,512,1);
    RMSE(1,epoch*2-1) = norm(col(xrlam(:,:,aaa)) - ref(:)) / SqrtPix;
    if epoch == 1
        original = imdb.images.noisy(:,:,1,aaa);
    end
    RMSEorig = norm(col(original) - ref(:)) / SqrtPix;
    fprintf('RMSE = %g\n ', RMSEorig);
    fprintf('RMSE = %g\n ', RMSE(1,epoch*2-1));
    ...
    for iii = 1 : nOuterIter
        fprintf('ULTRA Module %d of %d, beta = %d  , gamma = %d ', iii, nOuterIter, beta, gamma);
        xrla = zeros(size(ref(:),1),size(xrlam,3));
        parfor jjj = 1:size(xrlam,3)
            
            KapPatch = im2colstep(single(kappa(:,:,jjj)), PatSiz, SldDist); 
            ...
            switch KapType
                ...
                case 1  
                    Kappa_slice = Kappa;
                    D_R(:,jjj) = 2 * beta_recon * Kappa_slice(:) * maxLambda;  
                    % construct regularizer R(x)
                    R{jjj} = Reg_OST_Kappa(ig.mask, ImgSiz, PatSiz, SldDist, beta_recon, ...
                        gamma_recon, KapPatchh, mOmega, numBlock, CluInt);
            end
            temp = xrlam(:,:,jjj);
            tmp = denom(:,:,jjj);
            switch KapType
                case 0
                   xrla(:,jjj) = pwls_os_rlalm_l2normReg(temp(:), Ab, reshaper(sino(:,:,jjj), '2d'),  reshaper(wi(:,:,jjj), '2d'),...
                      R, tmp(:), D_R, mu_para, 'pixmax', inf, 'chat', 0, 'alpha', 1.999, 'rho', [],'niter', nIter);
%                    xrla(:,jjj) = pwls_os_rlalm(temp(:), Ab, reshaper(sino(:,:,jjj), '2d'),  reshaper(wi(:,:,jjj), '2d'),...
%                       R, tmp(:), D_R,  'pixmax', inf, 'chat', 0, 'alpha', 1.999, 'rho', [],'niter', nIter);
                case 1
                   xrla(:,jjj) = pwls_os_rlalm_l2normReg(temp(:), Ab, reshaper(sino(:,:,jjj), '2d'),  reshaper(wi(:,:,jjj), '2d'),...
                       R{jjj}, tmp(:), D_R(:,jjj), mu_para, 'pixmax', inf, 'chat', 0, 'alpha', 1.999, 'rho', [],'niter', nIter);
%                    xrla(:,jjj) = pwls_os_rlalm(temp(:), Ab, reshaper(sino(:,:,jjj), '2d'),  reshaper(wi(:,:,jjj), '2d'),...
%                       R{jjj}, tmp(:), D_R(:,jjj), 'pixmax', inf, 'chat', 0, 'alpha', 1.999, 'rho', [],'niter', nIter);
            end
         
            R{jjj}.nextOuterIter();
            
            xrlam(:,:,jjj) = reshape(xrla(:,jjj),512,512);
        end
        ttt = xrlam(:,:,aaa);
        AAA(1,iii) = norm(ttt(:) - ref(:)) / SqrtPix;
        fprintf('RMSE = %g \n ', AAA(1,iii));
        figure(66);imshow([ttt original ref],[800 1200])
    end
    xrlam = gpuArray(xrlam);
    RMSE(1,epoch*2) = AAA(1,nOuterIter);
    imdb.images.noisy = reshape(gather(xrlam), size(xrlam,1),size(xrlam,2),[],size(xrlam,3));
    info.RMSE = [RMSEorig RMSE];
    info.xx = ttt;
end
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof, FBPOutPut] = process_epoch(opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
% , Ab, sino, wi, R, denom, D_R, nIter, nOuterIter
...
for t=1:opts.batchSize:numel(subset)
    fprintf('%s: layer %02d: %3d/%3d: ', mode, epoch, ...
...
%         fprintf(' %s:%.3g', opts.errorLabels{i}, stats(i+2)/m) ;
    ...
    FBPOutPut(:,:,batch) = res(end-1).x(:,:,:,1) + lowRes(:,:,1);
end
