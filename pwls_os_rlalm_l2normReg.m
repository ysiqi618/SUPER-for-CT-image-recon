 function [x, cost] = pwls_os_rlalm_l2normReg(x, Ab, yi, wi, R, denom, D_R,mu,varargin)
...
% 	ticker(mfilename, iter, arg.niter)
	...
	for iset = 1:nblock
		...
		den = rho(k) * denom;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% + \|x - G(x)\|_2^2 penalty %%%%%%%%%
	    num = num + R.cgrad(x) + 2 * mu * (x - xinit) ;  % xinit = G_{theta}(x0)
            den = den + D_R + 2 * mu;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		x = x - relax * num ./ den;
		x = max(x, pixmin);
...
if arg.chat
  % calculate the cost value for each outer iteration
   cost = R.penal(Ab, x, wi, yi) ;
   cost(5) = mu * norm((x - xinit)).^2;
   cost(1) = cost(1)+ cost(5);
else
   cost = [];
end
