% -------------------------------------------------------------------------
function [net, info] = cnn_CT_denoising_ULTRA(imdb,xref,sino,kappa,wi,denom,beta_recon,gamma_recon,mOmega,...
    maxLambda,nIter,nOuterIter,isnufft,mu,maxthreads,overlap,layer_start,Layer, varargin)
% -------------------------------------------------------------------------
...
switch opts.networkType
    case 'simplenn', trainFn = @cnn_train_modified_wavultra_l2normReg;
    case 'dagnn', trainFn = @cnn_train_dag;
end

[net, info] = trainFn(net, imdb, getBatch(opts), xref,sino,kappa,wi,denom,beta_recon,gamma_recon,mOmega,maxLambda,...
    nIter,nOuterIter,isnufft,mu,maxthreads,overlap,layer_start,Layer,'expDir', opts.expDir, ...
    net.meta.trainOpts, ...
    opts.train, ...
