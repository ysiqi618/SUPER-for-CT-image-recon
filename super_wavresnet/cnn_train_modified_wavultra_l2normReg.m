function [net, info] = cnn_train_modified_wavultra(net, imdb, getBatch,xref,sino,kappa,wi,denom,...
    beta_recon,gamma_recon,mOmega, maxLambda,nIter,nOuterIter,isnufft,mu,maxthreads,overlap,...
    layer_start,Layer, varargin)
...
modelLayerPath           = @(layer) fullfile(opts.expDir, sprintf('net-Epoch%d-layer-%d.mat', opts.numEpochs,layer));
...
%     else
	...
	WavOut = inten2hu(cat(3,WavTrainOut,WavValOut));
	numslice = size(WavOut,3);
...
% 	KK = col2imstep(single(PP), ImgSiz, PatSiz, SldDist);
    ...
    mask = ig.mask;
	%% start pwls-ultra
	% poolobj = parpool([12,maxthreads]);
	parfor (islice = 1:numslice,maxthreads)
		fprintf('ULTRA Module %d of %d\n',islice, numslice);
	        KapPatch = im2colstep(kappa(:,:,islice), PatSiz, SldDist);
        	KapPatch = mean(KapPatch,1);
        	 % construct regularizer R(x)
       	        R = Reg_OST_Kappa(mask, ImgSiz, PatSiz, SldDist, beta_recon, gamma_recon, KapPatch, mOmega, numBlock, CluInt);
       	        KapPatch = col2imstep(single(repmat(KapPatch, prod(PatSiz), 1)), ImgSiz, PatSiz, SldDist);
                D_R = 2 * beta_recon * KapPatch(mask) * maxLambda;
      
		[xrla, info]=pwls_ultra_module(WavOut(:,:,islice),xref(:,:,islice),Ab,sino(:,:,islice),wi(:,:,islice),R,nIter,nOuterIter,...
            denom(:,:,islice),D_R,mu,SqrtPixNum,PatNum,ig);
		xultra(:,:,islice) = xrla;
		info_ultra{islice} = info;		
	end
	% delete(poolobj);
	save([opts.expDir '/pwls-ultra-layer-' num2str(layer) '.mat'],'xultra','info_ultra','-v7.3');
	imdb.images.data = cnn_wavelet_decon(double(hu2inten(xultra)), opts.wavLevel, opts.wavName);
	clear Ab sg ig xultra info_ultra;
%      end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
