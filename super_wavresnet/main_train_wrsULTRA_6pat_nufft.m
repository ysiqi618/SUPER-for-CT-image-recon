%%%%%%%%%%%%%%%%% training code for SUPER-WRN-ULTRA %%%%%%%%%%
...
expdir = sprintf('wavULTRA/mu%0.e_nufft_6pat_%diter_lr-5_imgs%d_PatSz%d_Overlap%d_BatSz%d_WgtDecay%.0e_GradMax%.0e/',...
    mu,nOuterIter,numslice,patchsize,overlap,batchsize,wgtdecay,gradMax);    % experiment name
if ~exist(expdir, 'dir'), mkdir(expdir); end
...
%%
printm('load training data ... \n');
% load('~/Desktop/SUPER-new/train100_val20.mat');
if layer_start ~= 1
load([expdir sprintf('/pwls-ultra-layer-%d.mat',layer_start-1)]);
xfbp_low = xultra; % using Layer 5 image
end
...
[net, info] = cnn_CT_denoising_ULTRA(imdb,xref,sino_low,kappa_low,wi_low,denom_low,beta_recon,gamma_recon,mOmega,...
    maxLambda,nIter,nOuterIter,isnufft, mu,maxthreads,overlap,layer_start,Layer, ...
