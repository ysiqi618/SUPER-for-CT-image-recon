function [net, info] = cnn_train_modified_wavep_l2normReg(net, imdb, getBatch,...
    xref,sino,kappa,wi,denom,l2b,delta,mu,maxthreads,overlap,layer_start,Layer,nIter, varargin)
...
%     for islice = 1:numslice
		fprintf('EP Module %d of %d\n',islice, numslice);
		isino = sino(:,:,islice);
		ikappa = kappa(:,:,islice);
		iwi = wi(:,:,islice);
		idenom = denom(:,:,islice);
		R = Reg1(sqrt(ikappa), 'beta', 2^l2b, 'pot_arg', pot_arg);
		xrla = WavOut(:,:,islice);
		ixref = xref(:,:,islice);
		[xrlalm_msk , info] = pwls_ep_os_rlalm_l2normReg(xrla(ig.mask), A, isino, R,mu,'wi', iwi, ...
	            'pixmax', inf, 'isave', 'last',  'niter', nIter, 'nblock', nblock, ...
	            'chat', 1, 'denom',idenom,'xtrue',ixref, 'mask', ig.mask);
		xep(:,:,islice) = ig.embed(xrlalm_msk);
	        info_ep{islice} = info;
	end
	% delete(poolobj);
	save([opts.expDir '/pwls-ep-layer-' num2str(layer) '.mat'],'xep','info_ep','-v7.3');
	imdb.images.data = cnn_wavelet_decon(double(hu2inten(xep)), opts.wavLevel, opts.wavName);
	clear A sg ig xep info_ep;
