% -------------------------------------------------------------------------
function [net, info] = cnn_CT_denoising_EP_l2normReg(imdb,xref,sino,kappa,wi,denom,l2b,delta,mu,...
    maxthreads,overlap,layer_start,Layer,nIter, varargin)
% -------------------------------------------------------------------------
...
switch opts.networkType
    case 'simplenn', trainFn = @cnn_train_modified_wavep_l2normReg;
    case 'dagnn', trainFn = @cnn_train_dag;
end

[net, info] = trainFn(net, imdb, getBatch(opts), xref,sino,kappa,wi,denom,l2b,delta,mu,...
    maxthreads,overlap,layer_start,Layer,nIter,...
    'expDir', opts.expDir, ...
    net.meta.trainOpts, ...
    opts.train, ...
